...
namespace FilterManager.Core
{
  internal static class Storage
  {
    ...
    private static Dictionary<T, bool> LoadDefs<T>(XElement? root) where T : Def
    {
      ...
      {
        var def = DefDatabase<T>.GetNamed(element.Name.LocalName, false);
        if (def == null)
        {
          Mod.Log($"Skipping missing def '{element.Name}'");
          continue;
        }
    ...
    {
      var element = new XElement(XElementPreset);
      element.Add(new XAttribute(XAttributeName, preset.Name));
      ...
      SerializeDefs(XElementSpecials, preset.Filters, element);
      SerializeDefs(XElementThings, preset.Things, element);
